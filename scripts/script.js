...
$('.thumbs span').click((e) => {
    ...
    $('.ourBoats .slider img').attr('src', `assets/img/${slider[e.target.id.match(/\d+/g).join('')].img}.jpg`);
    $('.ourBoats article h3').text(`${slider[e.target.id.match(/\d+/g).join('')].name}`);
    $('.ourBoats article p').text(`${slider[e.target.id.match(/\d+/g).join('')].description}`);
